...
void abc_module(RTLIL::Design *design, RTLIL::Module *current_module, std::string script_file, std::string exe_file,
		...
		const std::vector<RTLIL::Cell*> &cells, bool show_tempdir, bool sop_mode, bool abc_dress, std::string netlist)
{
...
#define fd_renumber(from, to) dup2(from, to)
		...
		if (!netlist.empty()) {
			Pass::call(mapped_design, "read_verilog " + netlist);
...
struct Abc_netlistPass : public Pass {
	Abc_netlistPass() : Pass("abc_netlist", "use ABC for technology mapping") { }
	void help() override
	{
		//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
		log("\n");
		log("    abc_netlist [options] [selection]\n");
		log("\n");
		...
		log("\n");
		log("    -netlist <file>\n");
		log("        use a given netlist file [.v] to back to yosys. The netlist must be\n");
		log("        flattened and the module name must is 'netlist'.\n");
		log("\n");
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		std::string lut_arg, luts_arg, g_arg, netlist;
		...
		for (auto mod : design->selected_modules())
		{
			...
			if (!dff_mode || !clk_str.empty()) {
				abc_module(design, mod, script_file, exe_file, liberty_files, genlib_files, constr_file, cleanup, lut_costs, dff_mode, clk_str, keepff,
						delay_target, sop_inputs, sop_products, lutin_shared, fast_mode, mod->selected_cells(), show_tempdir, sop_mode, abc_dress, netlist);
				continue;
			}
			...
			for (auto &it : assigned_cells) {
				...
				abc_module(design, mod, script_file, exe_file, liberty_files, genlib_files, constr_file, cleanup, lut_costs, !clk_sig.empty(), "$",
						keepff, delay_target, sop_inputs, sop_products, lutin_shared, fast_mode, it.second, show_tempdir, sop_mode, abc_dress, netlist);
				assign_map.set(mod);
			}
		}
